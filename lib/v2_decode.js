...
module.exports = function decode(buf, validate, receivedSig) {
	var socketDetails = {},
	...
	// gracefully handle healthcheck headers
	if (
		family === common.families.AF_UNSPEC &&
		protocol === common.protocols.UNSPEC
	) {
	...
	if (validate) {
		...
		receivedSig = receivedSig || buf.slice(0, sigLength);
		if (sigBytes.compare(receivedSig)) {
			throw new Error('proxy protocol: invalid signature received');
		}

		if ((buf[fields.ver_cmd] & 0xf0) !== common.V2_PROXY) {
			throw new Error('proxy protocol: invalid version or cmd received');
		}

		if (
			family !== common.families.AF_INET &&
			family !== common.families.AF_INET6
		) {
			throw new Error('proxy protocol: unsupported family received');
		}

		if (
			protocol !== common.protocols.STREAM &&
			protocol !== common.protocols.DGRAM
		) {
	...
	if (family == common.families.AF_INET) {
		...
		var ipv4Addr = common.ipv4Addr,
			addr = fields.addr;
		socketDetails.remoteAddress = decodeV4Address(
			buf,
			addr + ipv4Addr.src_addr
		);
		...
		var ipv6Addr = common.ipv6Addr,
			addr = fields.addr;
		socketDetails.remoteAddress = decodeV6Address(
			buf,
			addr + ipv6Addr.src_addr
		);
