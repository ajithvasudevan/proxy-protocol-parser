...
module.exports = function encode(socketDetails) {
	...
	if (family == 'IPv4') {
		var ipv4Addr = common.ipv4Addr,
			addr = fields.addr;
		buf = Buffer.alloc(addr + ipv4Addr.len);
		buf[fields.family] =
			common.families.AF_INET +
			(protocol === 'udp' ? common.protocols.DGRAM : common.protocols.STREAM);
		...
		var ipv6Addr = common.ipv6Addr,
			addr = fields.addr;
		buf = Buffer.alloc(addr + ipv6Addr.len);
		buf[fields.family] =
			common.families.AF_INET6 +
			(protocol === 'udp' ? common.protocols.DGRAM : common.protocols.STREAM);
...
function encodeV6Address(buf, offset, address) {
	var addr = new ipAddress.Address6(address),
		parsedAddress = addr.parsedAddress;

	for (var i = 0; i < parsedAddress.length; i++)
		buf.writeUInt16BE(
			Number.parseInt(parsedAddress[i], 16),
			offset + i * 2,
			true
		);
}
